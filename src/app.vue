      ...
      commands: [
        {
          ...
          value: { zh_CN: '润色', en_US: 'Polish' },
          autoSend: true,
        },
        {
          label: { zh_CN: '扩写', en_US: 'Expand' },
          value: { zh_CN: '扩写', en_US: 'Expand' },
          autoSend: true,
        },
        {
          label: { zh_CN: '自定义', en_US: 'Custom' },
          value: { zh_CN: '自定义', en_US: 'Custom' },
          autoSend: false,
        },
      ],
      async onMessage(payload, content) {
        console.log('assistant onMessage', { payload, content })
        await new Promise((resolve) => setTimeout(resolve, 600))

        if (payload.command === '润色' || payload.command === 'Polish') {
          return `${payload.input}（润色示例：表达更自然、语气更专业）`
        }

        if (payload.command === '扩写' || payload.command === 'Expand') {
